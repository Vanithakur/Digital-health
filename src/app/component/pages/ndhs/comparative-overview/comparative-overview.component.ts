...
export class ComparativeOverviewComponent implements OnInit {
    ...
    overviewBarChart() {
        ...
        this._common.getOverviewBarChart(data).subscribe((result) => {
            ...
            this.bar_chart_new = [];
            let bar_data = {
                text: result[0].country_name + ',' + result[1].country_name,
                comIncome:
                    result[0].percentage + '%,' + result[1].percentage + '%',
                compText: result[0].country_name + ',' + result[1].country_name,
                img: './assets/images/line.png',
                per: result[0].percentage,
            };

            if (result[0].percentage <= 25 && result[1].percentage <= 25) {
                this.bar_chart_new.push(bar_data);
            }
            if (
                result[0].percentage > 25 &&
                result[0].percentage <= 60 &&
                result[1].percentage > 25 &&
                result[1].percentage <= 60
            ) {
                this.bar_chart_new.push(bar_data);
            } else if (
                result[0].percentage > 60 &&
                result[0].percentage <= 80 &&
                result[1].percentage > 60 &&
                result[1].percentage <= 80
            ) {
                this.bar_chart_new.push(bar_data);
            } else if (
                result[0].percentage > 80 &&
    ...
    overviewRadarChart() {
        ...
        this._common.getOverviewRadarChart(data).subscribe((result) => {
    ...
    overviewBubbleChart() {
        ...
        this._common.getOverviewBubbleChart(data).subscribe((result) => {
    ...
    BarChart() {
        if (this.bar_chart) {
            ...
            chart.children.unshift(
                am5.Label.new(root, {
                    text: this.bar_chart[0].taxonomy_name,
                    fontSize: 15,
                    fontWeight: '500',
                    textAlign: 'center',
                    x: am5.percent(40),
